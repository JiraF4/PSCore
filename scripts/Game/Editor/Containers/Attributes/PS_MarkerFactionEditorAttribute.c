[BaseContainerProps()]
class PS_MarkerFactionEditorAttribute: SCR_BaseMultiSelectPresetsEditorAttribute
{
	override SCR_BaseEditorAttributeVar ReadVariable(Managed item, SCR_AttributesManagerEditorComponent manager)
	{
		...
		for(int i = 0; i < count; ++i)
		{
			...
			AddOrderedState(marker.GetMarkerVisibleForFaction(scrFaction));
		}
	...
	{		
		...
		for(int i = 0; i < count; ++i)
		{
			...
			marker.SetMarkerVisibleForFaction(scrFaction, GetOrderedState());
		}
	}
